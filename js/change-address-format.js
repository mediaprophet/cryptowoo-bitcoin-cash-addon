...
jQuery(document).ready(function ( $ ) {
    ...
    if(jQuery("#qrcode").length > 0 && CryptoWoo !== undefined ) {
        ...
        if(CryptoWoo.currency == 'XEC' && xecaddr.isLegacyAddress(CryptoWoo.payment_address)){
            jQuery('#qrcode').empty();
            var currentAddress = jQuery("#payment-address").text();
            var cashAddressWithoutLabel = xecaddr.toCashAddress(CryptoWoo.payment_address).replace(/ecash:/g,'');
...
function change_addr_XEC() {
    ...
    var currentPaymentAddress = xecaddr.toCashAddress(CryptoWoo.payment_address);
    ...
    var isLegacy = xecaddr.isLegacyAddress(currentPaymentAddress);
    var isCashAddress = xecaddr.isCashAddress(currentPaymentAddress);

    var addressWithoutLabel = currentPaymentAddress;

    if(isLegacy){
        addressWithoutLabel = xecaddr.toCashAddress(currentPaymentAddress).replace(/bitcoincash:/g,'');
        addr_format_button.text("Change to legacy-address");
    }
    else if (isCashAddress){
        addressWithoutLabel = xecaddr.toLegacyAddress(currentPaymentAddress).replace(/bitcoincash:/g,'');
